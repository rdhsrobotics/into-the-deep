...
class TestTwoSpec : HypnoticAuto({ robot ->
    single(" Testing 2+0 ") {
        ...
        navigateTo(Pose(-63.3, -45.4, 0.degrees)) //little up

        navigateTo(Pose(-23.3, -17.0, 0.degrees))//to the side a little for rotating




        navigateTo(Pose(-33.3, -10.33, 270.degrees))//pick up point
        ...
        robot.robot.intakeComposite.outtakeLevel(OuttakeLevel.lowBar2)
        (OuttakeLevel.lowBar2)
        navigateTo(Pose(0.0, -34.4, 0.degrees))
        robot.robot.intakeComposite.outtakeLevel(OuttakeLevel.bar2)
        (OuttakeLevel.bar2)
